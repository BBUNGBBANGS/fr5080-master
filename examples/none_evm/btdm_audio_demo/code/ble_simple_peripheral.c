...
void app_gap_evt_cb(gap_event_t *p_event)
{
    ...
    switch(p_event->type)
    {
        ...
        case GAP_SEC_EVT_SLAVE_ENCRYPT:
            LOG_INFO("slave[%d]_encrypted\r\n",p_event->param.slave_encrypt.conidx);
            if(pskeys.pmu_mode == 1){
                bt_reconnect(2, NULL);
            }
            break;

        case GAP_SEC_EVT_NC_DATA_IND:
            LOG_INFO("receive number compare data: %d\r\n", p_event->param.nc_data.nc_data);
            ble_sec_send_nc_dat_confirm(p_event->param.nc_data.conidx, true);
            break;
