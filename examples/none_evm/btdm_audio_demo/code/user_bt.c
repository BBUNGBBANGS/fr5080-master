...
os_timer_t bt_conn_delay_check_timer;
...
os_timer_t bt_inq_timer;

os_timer_t bt_role_change_timer;
void bt_role_change_timer_func(void *arg)
{
    if(user_bt_env->dev[0].remDev&&(user_bt_env->dev[0].conFlags == 0))
        bt_remove_acl_link(user_bt_env->dev[0].remDev);
}

void bt_inq_timer_func(void *arg)
{
    bt_start_inquiry(1,1);

}

static BtStatus reconnect_according_to_profile(BD_ADDR *bdaddr);
void bt_conn_delay_check_timer_func(void *arg)
{
    if((user_bt_env->dev[0].state == BT_CONNECTED) && (user_bt_env->dev[0].conFlags == 0)){
...
uint8_t remote_dev_type = 0;//0---phone,1---earphone
uint8_t user_evt_notify_enable = 0xff;

uint8_t music_player_open_flag = 1;
...
{
    if(user_bt_env->dev[0].state == BT_IDLE){
...
static BtStatus reconnect_according_to_profile(BD_ADDR *bdaddr)
{
    ...
    //uart_putc_noint('$');
    if(bt_get_hf_source_mode() == 1){
        if(remote_dev_type == 1){
            //connect to earphone
...
void bt_hf_evt_func(hf_event_t *event)
{
    ...
    switch(event->type){
        ...
        case HF_EVENT_SERVICE_CONNECTED:
        {
            ...
            buf = (uint8_t *)os_malloc(sizeof("AT+CGMI"));
            if(buf != NULL) {
                memcpy(buf, "AT+CGMI", sizeof("AT+CGMI"));
                hf_send_at_command(event->chan, (const char *)buf);
            }

            //hf_query_subscriber_number(event->chan);
        ...
        case HF_EVENT_CALLSETUP_IND:
            ...
            if((event->param.callSetup == 1)||(event->param.callSetup == 2)){
                hf_list_current_calls(event->chan);
            }
            break;
        case HF_EVENT_CURRENT_CALL_STATE:
            //const uint8_t *number = event->param.callListParms->number;
            //printf("current call:%c,%c,%c,%d\r\n",number[0],number[1],number[2],strlen(number));
            break;
        ///来电号码提示
        ...
        case HF_EVENT_SUBSCRIBER_NUMBER:

            break;
        case HF_EVENT_AT_RESULT_DATA:
            //HfAtData *atdata = (HfAtData *)event->param.data;
            //printf("at result:%x,%x\r\n",atdata->data,atdata->dataLen);
            break;
        ///HFP AT命令发送成
...
void bt_a2dp_evt_func(a2dp_event_t *event)
{
    ...
    switch(event->event){
        ...
        case A2DP_EVENT_STREAM_OPEN:
            ...
            if(bt_get_hf_source_mode() == 1){
                pskeys.app_data.misc_flag = 1; //source
            }
        ...
        case A2DP_EVENT_STREAM_STARTED:
            ...
            if(bt_get_a2dp_decorder_type() == A2DP_DEC_TYPE_SBC_DSP){
                dsp_open(DSP_LOAD_TYPE_A2DP_DECODER);
            }
    ...
    switch(event->type){
...
#endif
            os_timer_init(&bt_inq_timer,bt_inq_timer_func,NULL);
            break;

        case BTEVENT_INQUIRY_CANCELED:
            os_timer_stop(&bt_inq_timer);
            break;
        case BTEVENT_INQUIRY_COMPLETE:
            //搜索完成后，重新搜索附近设备，搜索个数限制为10个，搜索时间限制为10*1.28s
            os_timer_start(&bt_inq_timer, 300, 0);
            //bt_start_inquiry(1,1);
            break;
        ...
        case BTEVENT_LINK_CONNECT_IND:
            ...
            if(event->errCode == BEC_NO_ERROR){
                ...
                os_timer_stop(&bt_linkloss_timer);
                user_bt_env->dev[device_index].reconnecting = 0;
                bt_set_accessible_mode_nc(BAM_GENERAL_ACCESSIBLE,NULL);
                set_bt_reconnect_state(0);
                if(pskeys.pmu_mode == 1)
                {
                    //一键连接
                    os_timer_init(&bt_conn_delay_check_timer,bt_conn_delay_check_timer_func,NULL);
                    os_timer_start(&bt_conn_delay_check_timer,2000,false);
                }
            }
        ...
        case BTEVENT_LINK_DISCONNECT:
            ...
            os_timer_stop(&bt_role_change_timer);
            bt_connect_stage = 0;
            #if PBAP_FUC
            pbap_connect_flag = 0;
            #endif
        ...
        case BTEVENT_ROLE_CHANGE:
            LOG_INFO("role change:%d\r\n",event->param.newRole);
            if(bt_get_a2dp_type() == BT_A2DP_TYPE_SRC){
                os_timer_init(&bt_role_change_timer,bt_role_change_timer_func,NULL);
                os_timer_start(&bt_role_change_timer,3000,0);
            }
            break;
        default:
            break;
...
void bt_avrcp_evt_func(avrcp_event_t *event)
{
    ...
    switch(event->event){
            ...
            if((event->advOp == AVRCP_OP_GET_CAPABILITIES)
                ...
                if(event->param.adv.rsp.capability.info.eventMask & AVRCP_ENABLE_PLAY_STATUS_CHANGED){
                    ///注册播放状态改变事件，当TG播放状态改变时，产生AVRCP_EVENT_ADV_NOTIFY事件
                            avrcp_ct_register_notification(event->chnl, AVRCP_EID_MEDIA_STATUS_CHANGED, 0);
                }
                    if(event->param.adv.rsp.capability.info.eventMask & AVRCP_ENABLE_TRACK_CHANGED){
                        ...
                        avrcp_ct_register_notification(event->chnl, AVRCP_EID_TRACK_CHANGED, 0);
                    }
                }else{
                    //if(event->param.adv.rsp.capability.info.eventMask & AVRCP_ENABLE_VOLUME_CHANGED){
                        ///注册歌曲改变事件，当TG播放状态改变时，产生AVRCP_EVENT_ADV_NOTIFY事件
                        avrcp_ct_register_notification(event->chnl, AVRCP_EID_VOLUME_CHANGED, 0);
                    //}
                }
            }
            ///本地注册事件，返回的当前状态
            if(event->advOp == AVRCP_OP_REGISTER_NOTIFY) {
                ...
                if(event->param.adv.notify.event == AVRCP_EID_MEDIA_STATUS_CHANGED) {
                    if((event->param.adv.notify.p.mediaStatus == AVRCP_MEDIA_STOPPED)
                        ...
                        ///保存状态到本地，音乐暂停状态比A2DP中的suspend事件更早产生,  有利于APP状态判断
                        user_bt_env->dev[deviceNum].conFlags &= ~LINK_STATUS_MEDIA_PLAYING;
                    }
                    else {
        ...
        case AVRCP_EVENT_ADV_NOTIFY:
            ...
            //source 重新注册音量变化提醒
            if(event->param.adv.notify.event == AVRCP_EID_VOLUME_CHANGED) {
                LOG_INFO("vol notify: %d\r\n",event->param.adv.notify.p.volume);
                avrcp_ct_register_notification(event->chnl, AVRCP_EID_VOLUME_CHANGED, 0);
            }
            if(bt_get_a2dp_type() == BT_A2DP_TYPE_SINK){
                ///歌曲改变通知
...
//                            Parms->u.paramsRx.phonebookSize);
    ...
    case PBAP_EVENT_DATA_IND:
        if (offset == 0) {
            ...
            {
                left_size = 0;
                pbap_buffer = os_malloc(1024);
            }
        } 
        pbap_tx = os_malloc(Parms->u.dataInd.len + 9 + left_size);
    ...
    case PBAP_EVENT_TP_CONNECTED:
        ...
        user_bt_env->dev[0].conFlags |= LINK_STATUS_PBAP_CONNECTED;
        break;
...
void bt_hid_evt_func(void *chan, hid_event_t *event)
{
    ...
    switch(event->event) {
        case HIDEVENT_OPEN_IND:
            if(hid_accept_connection(chan) != BT_STATUS_PENDING) {
             //co_printf("accept connection failed.\r\n");
            }
        break;
        case HIDEVENT_OPEN:
            user_bt_env->dev[0].conFlags |= LINK_STATUS_HID_CONNECTED;
            user_bt_env->current_hid_index = 0;
            user_bt_env->dev[0].hid_chan = chan;
        break;
        ...
        case HIDEVENT_TRANSACTION_IND:
           // printf("trans ind = %d\r\n",event->ptrs.trans->type);
            trans = os_malloc(sizeof(HidTransaction));
            
            trans->resultCode = HID_RESULT_SUCCESS;
            hid_device_set_protocol_rsp(chan, trans);
        break;
    
        case HIDEVENT_TRANSACTION_COMPLETE:
            os_free((void *)(event->ptrs.trans));
        break;
    
        case HIDEVENT_INTERRUPT_COMPLETE:
            os_free(event->ptrs.intr->data);
            os_free((void *)(event->ptrs.intr));
        break;
     }
...
enum bt_state_t bt_get_state(void)
{
    ...
    else if((user_bt_env->dev[0].state == BT_CONNECTING)||(user_bt_env->dev[1].state == BT_CONNECTING)||(user_bt_env->dev[0].state == BT_CONNECT_PENDING)){
        ret_state = BT_STATE_CONNECTING;
    }
    else if(user_bt_env->dev[0].state == BT_DISCONNECTING){
        ret_state = BT_STATE_DISCONNECTING;
    }
    else{
...
bool bt_reconnect(enum bt_reconnect_type_t type,BD_ADDR *addr)
{
    ...
    if((bt_get_hf_source_mode() == 1)&&(addr == NULL)){
        remote_dev_type = pskeys.app_data.misc_flag;
    }
...
void bt_real_disconnect(void)
{
    ...
    if(user_bt_env->dev[i].conFlags&LINK_STATUS_PBAP_CONNECTED){
        pba_disconnect();
    }
}
...
void bt_disconnect(void)
{
    ...
    if(user_bt_env->dev[i].state == BT_CONNECTING){
        return;
    }
    if(user_bt_env->access_state == ACCESS_PAIRING){
        do_disconnect = false;
    ...
    if(user_bt_env->dev[i].state == BT_CONNECTED){
        user_bt_env->dev[i].state = BT_DISCONNECTING;
...
{
    uint8_t conn_flag = 0;
    uint8_t save_dev_flag = 0;
    
    if(bt_get_hf_source_mode() == 1){
        if(((user_bt_env->dev[dev_num].conFlags)&LINK_STATUS_HF_CONNECTED)
            ||(((user_bt_env->dev[dev_num].conFlags)&LINK_STATUS_AV_CONNECTED)&&((user_bt_env->dev[dev_num].conFlags)&LINK_STATUS_AVC_CONNECTED))){
            save_dev_flag = 1;
        }
    }
    else{
    ...
    */
    if(save_dev_flag == 1){
        ...
        //system_sleep_enable();
        uart_send("+CONN\r\n",7);
        LOG_INFO(":%02x,%02x,%02x,%02x,%02x,%02x\r\n",user_bt_env->dev[dev_num].bd.A[0],user_bt_env->dev[dev_num].bd.A[1],user_bt_env->dev[dev_num].bd.A[2],
                                                user_bt_env->dev[dev_num].bd.A[3],user_bt_env->dev[dev_num].bd.A[4],user_bt_env->dev[dev_num].bd.A[5]);
        ...
        if(do_disconnect == true){
            ...
            if(music_player_open_flag == 0){
                if(user_bt_env->dev[dev_num].conFlags&LINK_STATUS_AVC_CONNECTED){
                    avrcp_disconnect(user_bt_env->dev[dev_num].rcp_chan);
                }
                if(user_bt_env->dev[dev_num].conFlags&LINK_STATUS_AV_CONNECTED){
                    a2dp_close_stream(user_bt_env->dev[dev_num].pstream);
                }
            }
        }
...
//    uint8_t check_flag = 0;
    ...
    switch(event) {
        ...
        case USER_EVT_A2DP_STREAM_SUSPENDED:
            if(user_bt_state == USER_BT_STATE_A2DP_STARTED){
            user_bt_state = USER_BT_STATE_IDLE;
            }
