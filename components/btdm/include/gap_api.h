...
#define GAP_BOND_INFO_LTK_VALID                 0x01
#define GAP_BOND_INFO_IRK_VALID                 0x02
#define GAP_BOND_INFO_PEER_ADDR_VALID           0x04
...
typedef enum
{
    ...
    GAP_SEC_EVT_NC_DATA_IND,        //!< Link bond request number compare indide
} gap_event_type_t;
...
// number compare indide message
typedef struct
{
    uint8_t conidx;                 //!< Connection index
    uint32_t nc_data;               //!< number compare data
} gap_sec_evt_nc_data_ind_t;
typedef struct
{
    uint8_t conidx;
    uint8_t reason;
...
typedef struct
{
    ...
    union
    {
        ...
        gap_sec_evt_slave_t              slave_encrypt;   //!< Connection index of encrypted link, role as slave
        gap_sec_evt_nc_data_ind_t       nc_data;                //!< number compare data
    } param;
} gap_event_t;
...
typedef struct
{
    ...
    gap_mac_addr_t peer_identity_addr;  //!<identity addr of bonded peer device
    uint8_t peer_irk[16];       //!<peer_irk
    uint8_t peer_ltk[16];       //!<peer_irk
    uint8_t key_flags;                  //!<indicate which key is valid
...
/*********************************************************************
 * @fn      ble_sec_send_nc_dat_confirm
 *
 * @brief   accept number compare request or not.
 *
 * @param   conidx      - connection index.
 ...
 * @return  None.
 */
void ble_sec_send_nc_dat_confirm(uint8_t conidx, uint8_t accept);
 ...
 *
 * @brief   Configure BLE mac address.type
 *
 * @param   type  @ref ble_addr_type
 *
 * @return  None.
 */
void ble_set_addr_type(enum ble_addr_type addr_type);
/*********************************************************************
 * @fn      ble_set_rlsv_addr_renew_time
 *
 * @brief   Set resolveable or noneresolveable mac addr renew time
 *
 * @param   time - uint is scond
 *
 * @return  None.
 */
void ble_set_rlsv_addr_renew_time(uint16_t time);
